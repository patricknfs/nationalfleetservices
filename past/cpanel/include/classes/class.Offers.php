...
class Offers{
	...
	function processOffers($ofrID=0, $actionMode='ADD'){
		...
		$SqlSelect     =  "SELECT sno,FullImage FROM  " . TABLE_MAKES . "  WHERE sno='".(int)$makeid."'";
		$sqlResult     =  $this->dbObj->fun_db_query($SqlSelect);
		$makeResult    = $this->dbObj->fun_db_fetch_rs_object($sqlResult);
        $fullimage     = $makeResult->FullImage;
		if($fullimage)
		{
		$ofrArray['offer_image'] = $fullimage;
		}else
		{
		...
		if($actionMode=='ADD'){
			$fields = "";
			$fieldsVal = "";
			if($_POST['finance_type'] == 5){
				$ofrArray['finance_type'] = 1;
				foreach($ofrArray as $keys => $vals){
					$fields .= $keys . ", ";
				...
				$ofrArray['finance_type'] = 2;
				$fields1 = "";
				$fieldsVal1 = "";
				foreach($ofrArray as $keys1 => $vals1){
					$fields1 .= $keys1 . ", ";
					$fieldsVal1 .= "'" . fun_db_input($vals1). "', ";
				}
				$sqlInsert1 = "INSERT INTO " . TABLE_OFFERS . "(offer_id, ".$fields1." added_date) " ;
				$sqlInsert1 .= " VALUES(null, ".$fieldsVal1." '".date("Y-m-d H:i:s")."')";
				$this->dbObj->fun_db_query($sqlInsert1) or die("<font color='#ff0000' face='verdana' size='2'>Error: Unable to execute request!<br>Invalid Query On offer table.</font>");
			}else{
				foreach($ofrArray as $keys => $vals){
					$fields .= $keys . ", ";
					$fieldsVal .= "'" . fun_db_input($vals). "', ";
