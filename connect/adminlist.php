...
<table id="reminder">
	<tr>
		...
		<th width="13%">
            <a href="enqlist.php?status=<?=$status?>&sortfield=en_reminder" title="Click to sort on this column">Owner</a>
        </th>
        <th width="13%">
    ...
    while ($owners = mysql_fetch_array($result, MYSQL_ASSOC))
    {
		...
		echo '<td>'.date("d/m",strtotime($owners['en_reminder'])).' '.$owners['en_reminder_hour'].':00</td>';
