...
include('header.php');
	...
	if ($_SESSION['username'] == 'admin') {
		header( 'Location: http://www.fleetstreetltd.co.uk/connect/admin/index.php' ) ;
	};
...
echo "<br /><h3>No New Enquiries</h3>";
}
?>
...
$query = "SELECT en_no, en_ApplicantCompany, en_ApplicantForenameName, en_ApplicantSurname, en_VehicleMake, en_VehicleModel, en_ProposalType, en_reminder, en_ReminderNote, en_reminder_hour, en_owner FROM `enquiries` WHERE en_reminder <= CURdate() AND `en_reminder` <> 0 AND en_owner = '$theowner' ORDER by en_reminder, en_reminder_hour ASC";
...
echo ('<tr><td><a href="http://www.fleetstreetltd.co.uk/connect/enquiry-'.$line['en_no'].'">'.$line['en_no'].'</a></td><td><b>'.date("d/m/y",strtotime($line['en_reminder'])).' - '.$line['en_reminder_hour'].':00</b></td><td><a title="header=[] body=['.$line['en_ReminderNote'].']" href="http://www.fleetstreetltd.co.uk/connect/enquiry-'.$line['en_no'].'">'.$name.'</a></td><td>'.$vehicle.'</td><td>'.$line['en_ProposalType'].'</td></tr>'."\n");
}
...
?>
There are currently <a href="http://www.fleetstreetltd.co.uk/connect"><? echo $row['COUNT(en_owner)'] ?> available enquiries</a>
<?
}}
?>
</div>
