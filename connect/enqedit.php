...
<?php
  ...
  $line=@mysql_query("SELECT *
                       FROM enquiries
                       WHERE en_no='$en_no' AND (en_owner='$theowner' OR en_owner='')");
  if (!$line) {
...
<h3 class="grey">
<?=$en_ApplicantCompany?>&nbsp;<?=$en_ApplicantForenameName?>&nbsp;<?=$en_ApplicantSurname?>
</h3>

<br /><br />

<form action="<?=$_SERVER['PHP_SELF']?>" method="post" name="enquiry">
<table width="100%" id="enq">
...
<tr>
<td>Source / Region</td><td><input type="text" name="en_source" size="20" maxlength="100" value="<?=$en_source?>">
 / <input type="text" name="en_region" size="20" maxlength="100" value="<?=$en_region?>" placeholder="e.g. North East"></td>
</tr>
...
<tr><td>Type / Term / MPA / Maint</td><td>

<select name="en_ProposalType" id="en_ProposalType">
<?php
//Array of stages
$aProposalType = array("CH", "PCH", "CP", "FL", "LP", "HP", "Outright", "Other");
$dbProposalType = $en_ProposalType; // stored in database

foreach ($aProposalType as $en_ProposalType)
{
if($en_ProposalType == $dbProposalType) {
 echo "<option value=\"$en_ProposalType\" selected>$en_ProposalType</option>";
 } else
 {
  echo "<option value=\"$en_ProposalType\">$en_ProposalType</option>";
  }
}
?>
</select>

<input type="text" name="en_ContractTerm" size="10" maxlength="25" value="<?=$en_ContractTerm?>"><input type="text" name="en_ContractMileage" size="10" maxlength="25" value="<?=$en_ContractMileage?>">

<select name="en_Maintenance" id="en_Maintenance">
<?php
//Array of stages
$aMaintenance = array("None", "Maint", "Maint & Break");
$dbMaintenance = $en_Maintenance; // stored in database

foreach ($aMaintenance as $en_Maintenance)
{
if($en_Maintenance == $dbMaintenance) {
 echo "<option value=\"$en_Maintenance\" selected>$en_Maintenance</option>";
 } else
 {
  echo "<option value=\"$en_Maintenance\">$en_Maintenance</option>";
  }
}
?>
</select>
