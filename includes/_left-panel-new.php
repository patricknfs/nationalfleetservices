...
$manu_search = mysql_query("SELECT DISTINCT
`ratebook_lex`.`Manufacturer`
FROM
`ratebook_lex`
ORDER BY
`ratebook_lex`.`Manufacturer` ASC") or die(mysql_error()); 
 //$rows = mysql_num_rows($manu_search); 
...
<table width="200" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td class="search-bg"><form action="searchresult-new.php" method="get" name="searchfrm" id="search" onsubmit="javascript: return funValidation();">
 ...
 { ?>
          <option value="<? echo($info['Manufacturer']) ?>" ><? echo($info['Manufacturer']) ?></option>
          <? } ?>
        </select>
