...
<div class="row">
	<form method="post" action="<?php echo safe_output($_SERVER['REQUEST_URI']); ?>">
		...
		<div class="col-md-9">
			...
			<div class="well well-sm">
				<?php if ($config->get('registration_enabled')) { ?>
						<p><?php echo safe_output($language->get('Name')); ?><br /><input required type="text" name="name" value="<?php if (isset($_POST['name'])) echo safe_output($_POST['name']); ?>" /></p>
						<p><?php echo safe_output($language->get('Username')); ?><br /><input required type="text" name="username" value="<?php if (isset($_POST['username'])) echo safe_output($_POST['username']); ?>" /></p>
						<p><?php echo safe_output($language->get('Email')); ?><br /><input required type="email" name="email" value="<?php if (isset($_POST['email'])) echo safe_output($_POST['email']); ?>" /></p>					
						<p><?php echo safe_output($language->get('Password')); ?><br /><input required type="password" name="password" value="" autocomplete="off" /></p>
						<p><?php echo safe_output($language->get('Password Again')); ?><br /><input required type="password" name="password2" value="" autocomplete="off" /></p>
