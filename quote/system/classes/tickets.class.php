...
class tickets {
	...
	public function get($array = NULL) {
		...
		if (isset($array['get_other_data']) && ($array['get_other_data'] == true)) {
			...
			$query .= " LEFT JOIN 
				(
					SELECT tn.user_id, tn.name, tn.email, tn.id, tn.ticket_id FROM $tables->ticket_notes AS tn 					
					JOIN (
						SELECT MAX(tn_temp.id) as id, tn_temp.ticket_id FROM $tables->ticket_notes AS tn_temp GROUP BY tn_temp.ticket_id
		...
		if (isset($array['like_search'])) {
			$query .= " AND (t.id LIKE :like_search OR t.subject LIKE :like_search OR t.description LIKE :like_search OR t.name LIKE :like_search OR t.email LIKE :like_search OR t.id IN (SELECT ticket_id FROM $tables->ticket_notes tn2 WHERE tn2.site_id = :site_id AND t.id = tn2.ticket_id AND tn2.private = 0 AND (tn2.description LIKE :like_search OR tn2.name LIKE :like_search OR tn2.email LIKE :like_search)))";
		}
		if (isset($array['access_key'])) {
	...
	public function merge($array) {
		...
		if (!empty($primary_tick)) {
			...
			foreach($primary_files as $file) {
				$storage->add_file_to_ticket(array('ticket_id' => $id, 'file_id' => $file['id']));
			}
			unset($file);
