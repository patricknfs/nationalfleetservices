...
class users {
	...
	public function get($array = NULL) {
		...
		if (isset($array['notification_department']['type']) && isset($array['notification_department']['id'])) {
			$query .= " AND (u.user_level IN (SELECT user_level FROM $tables->user_levels_to_department_notifications WHERE type = :notification_department_type AND department_id = :notification_department_id)";
		...
		if (isset($array['name']) && !empty($array['name'])) {
			if (!isset($array['email']) || empty($array['email']) || check_email_address($array['email'])) {
				if (empty($array['username']) || !$this->check_username_taken(array('username' => $array['username']))) {				
					...
					$array['email_notifications'] = 1;
					if (isset($array['email_notifications'])) {
						$add_array['email_notifications'] = $array['email_notifications'];					
					}
					if (isset($array['email'])) {
					...
					if ($array['allow_login'] == 1) {
						...
						if (isset($array['user_level'])) {
							$add_array['user_level']		= (int) $array['user_level'];
							
							if (SAAS_MODE) {
								if ($add_array['user_level'] != 0 && $add_array['user_level'] != 1) {
									if ($this->count(array('user_levels' => array('2,3,4,5,6'))) >= SAAS_MAX_USERS) {
										$create_user				= false;
